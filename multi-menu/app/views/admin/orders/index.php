...
<div class="max-w-5xl mx-auto p-4">
  <div class="flex items-center justify-between mb-4">
    ...
    <a class="px-3 py-2 rounded-xl border" href="<?= base_url("admin/{$activeSlug}/dashboard") ?>">← Voltar</a>
  </div>

  <form class="mb-4 flex gap-2" method="get" action="<?= base_url('admin/' . e($activeSlug) . '/orders') ?>">
      ...
      <tbody>
        <?php foreach ($orders as $o): ?>
          <tr class="border-t">
            ...
            <td class="p-2">
              <a class="underline" href="<?= base_url('admin/' . e($activeSlug) . '/orders/show?id='.(int)$o['id']) ?>">ver</a>
            </td>
          </tr>
        <?php endforeach; ?>
        <?php if (empty($orders)): ?>
